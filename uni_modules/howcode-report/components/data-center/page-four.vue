<template>
	<view class="content">
		<scroll-view class="data_body" scroll-y :style="{height:scrollHeight}" @scroll="toScroll">
			<template v-if="true">
				<!-- 会员成长数据-->
				...
				<!-- 新增会员排行榜 -->
				<view v-if="RankData" class="friend_operate">
					<view class="title">新增会员排行榜</view>
					<progress-bar :isPC="isPC" :isRank="isRank" :content="RankData" @updateRanking="updateRanking" />
				</view>
				
				<!-- <view class="update_data" @click="update_data" style="background-image: linear-gradient(to top right,#0081ff,#1cbbb4)" :animation="animationData">刷新</view> -->
			</template>
			<template v-else>
