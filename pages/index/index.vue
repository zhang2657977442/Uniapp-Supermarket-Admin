<template>
	<view>
		...
		>
			...
			<cover-view class="cameraBtn" @click="takePhoto"><cover-view class="cameraBtn2"></cover-view></cover-view>
			...
			<cover-view class="bottomBg" v-if="imgList.length > 0">
				<cover-view>
					<cover-view @click="ViewImage(index)" class="imgBox" v-for="(item, index) in imgList" :key="index">
						...
						<cover-view class="cu-tag" @tap.stop="DelImg" :data-index="index">
							<cover-image
								class="iconClose"
								src="https://cdn.zhoukaiwen.com/icon_close.png"
								mode="aspectFill"
							></cover-image>
						</cover-view>
					</cover-view>
				...
				<cover-view @click="checkOut" class="report">结账（{{ imgList.length }}）</cover-view>
			</cover-view>
		</camera>
		...
		uni.getLocation({
			...
			success: function(res) {
				...
				qqmapsdk = new QQMapWX({
					key: 'H2RBZ-76OWD-BUO4P-PDBMT-TXUTS-PABLR' //自己申请的key
				})
			...
			uni.chooseImage({
				success: chooseImageRes => {
					...
					const uploadTask = uni.uploadFile({
						url: '/api/file_upload', //仅为示例，非真实的接口地址
